...
import { ExecutionStatus, Prisma } from '@/generated/prisma/client'
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        // 构建查询条件
        const publishedVersions = app.publishedVersions.map(v => v.id)

        // 如果没有发布版本，返回空列表 (使用 api 调用流程时，必须要先发布再执行)
        ...
        const where: Prisma.AppExecutionWhereInput = {
            publishedAppId: { in: publishedVersions },
        }
