...
export class ConditionExecutor extends BaseNodeExecutor<ConditionNodeConfig> {
    readonly type = 'condition' as NodeKind
    ...
    protected async doExecute(
        ...
        const userPrompt = `请分析以下用户输入，判断其意图用户输入：${inputText} 请返回 JSON 格式的结果。`
        const startTime = Date.now()
        const llm = new ChatOllama({ model: resolvedConfig.model, temperature: 0 })
    ...
    override validate(config: ConditionNodeConfig): ValidationResult {
        const errors: string[] = []
